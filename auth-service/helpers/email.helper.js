                ...
                <tr>
                    <td align="center">
                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width: 600px; background-color: ${BRAND.panel}; border: 1px solid ${BRAND.border}; border-radius: 12px; overflow: hidden;">
                            <tr>
                                <td style="padding: 24px 32px; background: linear-gradient(135deg, ${BRAND.panelLight} 0%, ${BRAND.panel} 100%); border-bottom: 3px solid ${BRAND.primary}; text-align: center;">
                                    <img src="${process.env.FRONTEND_URL || 'http://localhost:5173'}/indelogo.png" alt="Logo INDE" style="max-height: 50px; margin-bottom: 12px; display: inline-block; background-color: #0aa5b5; padding: 5px; border-radius: 5px;" />
                                    <h1 style="margin: 0; color: ${BRAND.text}; font-size: 26px; line-height: 1.25; font-weight: 700;">${APP_NAME}</h1>
                                </td>
                            </tr>
                            <tr>
        ...
        contentHtml = `
            <div style="text-align: center; margin-bottom: 20px;">
                <p style="color: #666; font-size: 16px;">
                    Tu cuenta ha sido creada por un administrador. <br><br>
                    Tu usuario es: <strong>${username}</strong><br>
                    Tu contraseña temporal es: <strong>${tempPassword}</strong>
                </p>
            </div>
            <div style="text-align: center;">
                <a href="${activationLink}" style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px; font-weight: bold;">
                    Activar Cuenta
                </a>
            </div>
            <p style="text-align: center; margin-top: 20px; color: #999; font-size: 14px;">
                Por motivos de seguridad, te pediremos que cambies tu contraseña al iniciar sesión por primera vez.
            </p>
        `;
    } else {
        contentHtml = `
            <div style="text-align: center; margin-bottom: 20px;">
                <p style="color: #666; font-size: 16px;">
                    Gracias por registrarte en nuestra plataforma. Para completar tu registro, por favor activa tu cuenta haciendo clic en el siguiente botón:
                </p>
            </div>
            <div style="text-align: center;">
                <a href="${activationLink}" style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px; font-weight: bold;">
